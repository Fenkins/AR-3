...
export async function executeResearchCycle(spaceId: string, stageId?: string): Promise<any> {
  ...
  const state = getExecutionState(spaceId)
  const currentCycle = state?.currentCycle ?? (space as any).currentCycle ?? 1

  // Generate variants for this stage if none exist yet (integrated into main pipeline)
  if (state) {
    let stageVariants = state.variants.filter(v => v.stageId === currentStage.id && ((v as any).cycleNumber || 1) === currentCycle)
    ...
    if (shouldRegenerateStageVariants) {
      ...
      state.variants = state.variants.filter(v => !(v.stageId === currentStage.id && ((v as any).cycleNumber || 1) === currentCycle))
      stageVariants = []
    }
  ...
  if (state && state.variants && state.variants.length > 0) {
    const pendingVariant = state.variants.find(v => v.stageId === currentStage.id && ((v as any).cycleNumber || 1) === currentCycle && v.status === 'PENDING')
    if (pendingVariant) {
  ...
  debugLog(`[executeResearchCycle] Calling AI for stage: ${curre…30024 tokens truncated…=> v.id !== vid)
  ...
  if (space.stepsPerVariantMode === 'auto') {
    try {
      const activeCycle = state.currentCycle || (space as any).currentCycle || 1
      const stageVariants = updatedVariants.filter(v => v.stageId === variant.stageId && ((v as any).cycleNumber || 1) === activeCycle)
