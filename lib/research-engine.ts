...
6. If using downloaded models, load from AR3_MODEL_LOCAL_DIR or AR3_MODEL_CACHE_DIR -- never from hardcoded /tmp paths or remote URLs
...
export async function executeResearchCycle(spaceId: string, stageId?: string): Promise<any> {
  ...
  try {
    ...
    // Route GPU-enabled stages and autonomous preparation stages through the
    // worker whenever the space has GPU execution enabled.
    // The LLM's response text contains GPU commands in structured format
    if (shouldRouteStageThroughGpu(currentStage.name, currentStage.gpuEnabled, useGpu)) {
...
async function executeVariant(variant: Variant, spaceId: string, stageName: string, gpuEnabled: boolean): Promise<Variant> {
  ...
  const useGpu = shouldRouteStageThroughGpu(stageName, gpuEnabled, (space as any).useGpu ?? false)
  const useSystemRamOffload = useGpu && ((space as any).useSystemRamOffload ?? false)

  variant.status = 'RUNNING'
  await updateVariantDb(variant.id, { status: 'RUNNING' })
  ...
  for (const step of variant.steps) {
    ...
    if (useSystemRamOffload) {
      researchContext += `\n══════ MEMORY OFFLOAD MODE ══════
This space has system-RAM model offload enabled. When loading HuggingFace/transformers models, prefer CPU/RAM offload over hard CUDA placement:
- use device_map="auto", low_cpu_mem_usage=True, offload_state_dict=True
- set offload_folder=os.environ["AR3_RAM_OFFLOAD_DIR"] when provided; never hardcode /tmp offload directories
- set max_memory from AR3_TRANSFORMERS_MAX_MEMORY_JSON when available
- avoid model.to("cuda") after from_pretrained; move only input tensors to the model's first CUDA device when needed
- if loading still fails, print the exact OOM/VRAM/RAM error as JSON evidence instead of replacing it with prose
The GPU worker also applies a runtime safety net for common transformers loaders in this mode.
═════════════════════════════════════════════════════════════\n`
    }
          ...
          const gpuResponse = await fetchWithTimeout(`${internalGpuApiBase}/api/jobs/gpu`, {
            ...
            body: JSON.stringify({
              ...
              context: JSON.stringify({
                ...
                memoryOffload: {
                  enabled: useSystemRamOffload,
                  mode: useSystemRamOffload ? 'system_ram_transformers_offload' : 'vram_only',
                },
              }),
            }),
          }, 30000)
  ...
  async function loop() {
    ...
    try {
      ...
      if (reconciledGpuSteps > 0) {
        ...
        try {
          updateExecutionState(spaceId, { variants: await loadVariantsFromDb(spaceId) })
        } catch (err: any) {
          debugLog(`[startBackgroundLoop] Failed to reload variants after GPU reconciliation: ${err.message}`)
        }
      }
      ...
      if (recoveredStaleSteps > 0) {
        ...
        try {
          ...
          debugLog(`[startBackgroundLoop] Failed to reload variants after stale step recovery: ${err.message}`)
        }
      }
      ...
      if (recoveredStaleVariants > 0) {
        ...
        try {
          ...
          debugLog(`[startBackgroundLoop] Failed to reload variants after stale variant recovery: ${err.message}`)
        }
      }
...
export async function resumeSpace(spaceId: string) {
  ...
  try {
    const reconciledGpuSteps = await reconcileCompletedGpuJobsForRunningSteps(spaceId)
    if (reconciledGpuSteps > 0) {
      debugLog(`[resumeSpace] Reconciled ${reconciledGpuSteps} completed GPU job(s) from stalled RUNNING step(s)`)
      variants = await loadVariantsFromDb(spaceId)
    }
  } catch (err: any) {
    debugLog(`[resumeSpace] GPU job reconciliation failed: ${err.message}`)
  }

  try {
    const recoveredStaleSteps = await recoverStaleRunningStepsWithoutGpuJobs(spaceId)
    if (recoveredStaleSteps > 0) {
      debugLog(`[resumeSpace] Recovered ${recoveredStaleSteps} stale RUNNING step(s) without active GPU jobs`)
      variants = await loadVariantsFromDb(spaceId)
    }
  } catch (err: any) {
    debugLog(`[resumeSpace] Stale running step recovery failed: ${err.message}`)
  }

  try {
    const recoveredStaleVariants = await recoverStaleRunningVariantsWithoutActiveSteps(spaceId)
    if (recoveredStaleVariants > 0) {
      debugLog(`[resumeSpace] Recovered ${recoveredStaleVariants} stale RUNNING variant(s) without active steps`)
      variants = await loadVariantsFromDb(spaceId)
    }
  } catch (err: any) {
    debugLog(`[resumeSpace] Stale running variant recovery failed: ${err.message}`)
  }
