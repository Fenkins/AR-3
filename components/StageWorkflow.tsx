  ...
  const fetchSpaceData = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      if (data.space) {
        ...
        const serverCurrentCycle = data.space.cycleSummary?.activeCycle || data.space.displayCycle || data.execution?.currentCycle || data.space.currentCycle || 1
        setCurrentCycle(serverCurrentCycle)
        setSelectedCycle(prev => prev || serverCurrentCycle)
        ...
        const activeCycle = selectedCycle || serverCurrentCycle
        const stageVars = allVars.filter((v: any) => v.stageId === selectedStageId && (v.cycleNumber || 1) === activeCycle)
            ...
            <div className="mt-6 space-y-6">
              <div className="space-y-3">
                {setupSteps.map((step, i) => {
                  const stepProgress = setupProgre…4003 tokens truncated…nt.status.toLowerCase()}</span>
