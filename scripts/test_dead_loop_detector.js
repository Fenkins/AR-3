...
const repeatedWrappedStdoutMetricCompletion = (id, accuracy = 0.42, grade = 0) => ({
  id,
  stageId: 'stage_3',
  name: `Implementation ${id}`,
  status: 'COMPLETED',
  ...
  steps: [{
    ...
    result: JSON.stringify({
      ...
      job_id: `job-${id}`,
      stdout: [
        `workbench=/tmp/ar3-workbenches/${id}`,
        `metrics={"accuracy":${accuracy},"cuda_available":true,"elapsed_ms":${1000 + id.charCodeAt(0)},"artifact_path":"/tmp/ar3-workbenches/${id}/metrics.json"}`,
      ].join('\n'),
      stderr: '',
    }),
...
{
  const first = variantProgressSignature(repeatedWrappedStdoutMetricCompletion('a'))
  const second = variantProgressSignature(repeatedWrappedStdoutMetricCompletion('b'))
  assert.equal(first, second, 'wrapped stdout metric output should create the same progress signature despite noisy job fields')
}

{
  const assessment = assessDeadLoop([
    repeatedWrappedStdoutMetricCompletion('a'),
    repeatedWrappedStdoutMetricCompletion('b'),
    repeatedWrappedStdoutMetricCompletion('c'),
  ], 'stage_3')
  assert.equal(assessment.stuck, true)
  assert.equal(assessment.repeatedCount, 3)
  assert.match(assessment.reason, /no grade improvement/)
...
{
  const assessment = assessDeadLoop([
    repeatedWrappedStdoutMetricCompletion('a', 0.42),
    repeatedWrappedStdoutMetricCompletion('b', 0.43),
    repeatedWrappedStdoutMetricCompletion('c', 0.44),
  ], 'stage_3')
  assert.equal(assessment.stuck, false)
}
